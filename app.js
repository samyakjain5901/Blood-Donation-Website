...
mongoose.connect(
  "mongodb+srv://yush:" +
    ...
    "@cluster0.dgbfh.mongodb.net/myFirstDatabase?retryWrites=true&w=majority",
  {
...
const oAuth2Client = new google.auth.OAuth2(
  process.env.CLIENT_ID_NODEMAILER,
  process.env.CLIENT_SECRET_NODEMAILER,
  REDIRECT_URI
);
...
var transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    clientId: process.env.CLIENT_ID_NODEMAILER,
    clientSecret: process.env.CLIENT_SECRET_NODEMAILER,
  ...
  new GoogleStrategy(
    {
      ...
      callbackURL:
        "https://mighty-bastion-60878.herokuapp.com/auth/google/secrets",
  ...
  new FacebookStrategy(
    {
      ...
      callbackURL:
        "https://mighty-bastion-60878.herokuapp.com/auth/facebook/secrets",
...
app.post("/verify/email", function (req, res) {
  ...
  transporter.sendMail(mailOptions, function (error, info) {
    if (error) {
      console.log("the mail could not be sent", error);
...
app.get(
  ...
  function (req, res) {
    console.log("yo,", "i am working fine", req.user);
    res.redirect("/");
  }
);
...
app.get("/donors/mystatus", function (req, res) {
  ...
  else res.send({ message: "You are not even logged in" });
});
...
app.get("/donors/findme", function (req, res) {
  console.log(req.user);
  if (!req.user) res.send("No user is logged in");
  else {
    Donor.findOne({ "userDetails._id": req.user._id }, function (err, user) {
      if (user) {
        res.send({
          message: "found the donor",
          Details: user,
        });
      } else {
        res.send({
          message: "couldn't find the donor",
          Details: user,
        });
      }
    });
...
app.post("/donors/modifydetails", function (req, res) {
  ...
  Donor.findOneAndUpdate(
    ...
    function (err, doc) {
      ...
      return res.send({
        message: "successfully modified the document and saved it .",
      });
    }
  );
});
  ...
  BloodRequest.find(
    ...
    function (err, FoundDocuments) {
      if (FoundDocuments.length >= 3) {
        //ask for delete and save
        res.send({
          message: "there already exists 3 requests",
          request: FoundDocuments,
        });
      } else {
        //successfully save the document to the device
      ...
      Donor.find(
        ...
        function (err, Donors) {
          const txt =
            ...
            " if you are available and wish to help which will be highly appreciated.";
          Donors.forEach((element) => {
            ...
            transporter.sendMail(mailOptions, function (error, info) {
              if (error) {
                console.log("could'nt send email to ", mailOptions.to);
              } else {
          ...
          res.send({
            message:
              "An email has been sent to all the donors near you for your case and we hope you find a person and get well soon!!",
            donors: Donors,
          });
        }
      );
...
app.post("/bloodrequest/particularrequest", function (req, res) {
  const id = req.body.id;
  BloodRequest.findById(id, function (err, result) {
    if (result) {
      res.send({
        message: "found a matching request",
        request: result,
      });
    } else {
      res.send({
...
if (process.env.NODE_ENV === "production") {
  ...
  app.use(
    express.static(path.join(__dirname, "./bloodDonationFrontEnd/build"))
  );
  app.get("*", function (req, res) {
    res.sendFile(
      path.join(__dirname, "./bloodDonationFrontEnd", "build", "index.html")
    );
  });
} else {
  app.get("/", function (req, res) {
